...
import test.mayaUtils as testMayaUtils
...
def test1():
    ...
    # New scene
    testMayaUtils.reinitializeScene(frameRange=(start, end))

    # create camera
    tfm, shp = testMayaUtils.createCameraAsset('renderCamera')
    ...
    testMayaUtils.saveTestFile('test1_before')
    ...
    # save the scene
    testMayaUtils.saveTestFile('test1_after')
    return
...
def test2():
    ...
    # create cameras
    tfm1, shp1 = testMayaUtils.createCameraAsset('camera1')
    tfm2, shp2 = testMayaUtils.createCameraAsset('camera2')
    tfm3, shp3 = testMayaUtils.createCameraAsset('camera3')
    tfm4, shp4 = testMayaUtils.createCameraAsset('camera4')
    ...
    testMayaUtils.createRigAsset(name='characterSphereName',
                                 geomList=[sphere], ctrlList=[ctrl])

    # create cube rig
    cube = maya.cmds.polyCube()[0]
    ctrl = maya.cmds.circle(nr=(0, 1, 0), radius=2)[0]
    testMayaUtils.createRigAsset(name='characterCubeName',
                                 geomList=[cube], ctrlList=[ctrl])

    # save scene before test
    testMayaUtils.saveTestFile('test2_before')
    ...
    # save the scene
    testMayaUtils.saveTestFile('test2_after')
    return
...
def test3():
    ...
    # save file
    testMayaUtils.saveTestFile('test3_before')
    ...
    # save the scene
    testMayaUtils.saveTestFile('test3_after')
    return
...
def test4():
    ...
    # create shaders
    testMayaUtils.createShaderAsset('redPaint',
                                    nodeType='blinn',
                                    color=(0.7, 0.2, 0.2))
    testMayaUtils.createShaderAsset('bluePaint',
                                    nodeType='blinn',
                                    color=(0.2, 0.2, 0.7))
    testMayaUtils.createShaderAsset('grey',
                                    nodeType='lambert',
                                    color=(0.5, 0.5, 0.5))
    testMayaUtils.createShaderAsset('glass',
                                    nodeType='phong',
                                    color=(1.0, 1.0, 1.0))
    testMayaUtils.saveTestFile('test4_before')

    # do checks
    lib.run()

    # save the scene
    testMayaUtils.saveTestFile('test4_after')
    return
...
def test5():
    ...
    # create shaders
    uvShd, uvShdGrp = testMayaUtils.createShaderAsset('uvTest', nodeType='lambert',
                                                      color=(0.5, 0.5, 0.5))
    woodShd, woodShdGrp = testMayaUtils.createShaderAsset('wood', nodeType='blinn',
                                                          color=(0.5, 0.5, 0.5))

    # create uv shader textures
    testUvs = os.path.join(testBaseDir, 'uvTestTexture.jpg')
    uvTx = testMayaUtils.createTextureAsset('uvTestTexture', nodeType='file', path=testUvs)
    testMayaUtils.connectTextureToShader(uvTx, uvShd, 'color')
    ...
    woodColorTx = testMayaUtils.createTextureAsset('woodColor', nodeType='file', path=woodColorPath)
    woodSpecTx = testMayaUtils.createTextureAsset('woodSpec', nodeType='file', path=woodSpecPath)
    woodNormTx = testMayaUtils.createTextureAsset('woodNorm', nodeType='file', path=woodNormPath)
    woodBumpTx = testMayaUtils.createTextureAsset('woodBump', nodeType='file', path=woodBumpPath)
    testMayaUtils.connectTextureToShader(woodColorTx, woodShd, 'color')
    testMayaUtils.connectTextureToShader(woodSpecTx, woodShd, 'specularColor')
    testMayaUtils.saveTestFile('test5_before')

    # do checks
    lib.run()

    # save the scene
    testMayaUtils.saveTestFile('test5_after')
    return
...
def test6():
    ...
    uvShd, uvShdGrp = testMayaUtils.createShaderAsset('uvTest', nodeType='lambert',
                                                      color=(0.5, 0.5, 0.5))
    # testMayaUtils.assignShaderAsset([cube], uvShdGrp)
    testMayaUtils.assignObjectListToShader(objList=[cube], shader=uvShd)

    # create uv shader textures
    testUvs = os.path.join(testBaseDir, 'uvTestTexture.jpg')
    uvTx = testMayaUtils.createTextureAsset('uvTestTexture', path=testUvs)
    testMayaUtils.connectTextureToShader(uvTx, uvShd, 'color')

    # save scene before test
    testMayaUtils.saveTestFile('test6_before')

    # do checks
    lib.run()

    # save the scene
    testMayaUtils.saveTestFile('test6_after')
    return
...
def test7():
    ...
    sphere = maya.cmds.polySphere()[0]
    geomList = [sphere]
    geomSet = testMayaUtils.createGeometryAsset(name='sphereGeom',
                                                geomList=geomList)
    testMayaUtils.saveTestFile('test7_before')
    ...
    # save the scene
    testMayaUtils.saveTestFile('test7_after')
    return
...
def test8():
    ...
    # create camera
    for i in xrange(100):
        tfm, shp = testMayaUtils.createCameraAsset('renderCamera1')
        maya.cmds.setAttr(shp + '.renderable', 1)
        maya.cmds.setKeyframe(tfm, at='translateZ', t=start, v=4.0)
        maya.cmds.setKeyframe(tfm, at='translateZ', t=end, v=6.0)
        maya.cmds.setKeyframe(tfm, at='rotateX', t=start, v=0.0)
        ...
        # create geometry
        sphere = maya.cmds.polySphere()[0]
        geomSet = testMayaUtils.createGeometryAsset(name='sphereGeom1',
                                                    geomList=[sphere])
        ...
        # create shaders
        uvShd, uvShdGrp = testMayaUtils.createShaderAsset('uvTest',
                                                          nodeType='lambert',
                                                          color=(0.5, 0.5, 0.5))
        # testMayaUtils.assignShaderAsset([cube], uvShdGrp)
        testMayaUtils.assignObjectListToShader(objList=[cube], shader=uvShd)
        testMayaUtils.assignObjectListToShader(objList=[sphere], shader=uvShd)

        # create uv shader textures
        testUvs = os.path.join(testBaseDir, 'uvTestTexture.jpg')
        uvTx = testMayaUtils.createTextureAsset('uvTestTexture1', path=testUvs)
        testMayaUtils.connectTextureToShader(uvTx, uvShd, 'color')

    # save scene before test
    testMayaUtils.saveTestFile('test8_before')

    # do checks
    lib.run()

    # save the scene
    testMayaUtils.saveTestFile('test8_after')
    return
